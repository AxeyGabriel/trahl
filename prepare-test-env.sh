...
$FFMPEG -y -f lavfi -i testsrc=size=1280x720:rate=30 -f lavfi -i sine=frequency=1000:sample_rate=44100 -t 120 -c:v libx264 -preset veryslow -crf 23 -c:a aac "$TARGET_DIR/red_320x240_h264_big.mp4"
...
local remuxed = cachedir .. "/remux.mkv"
local filename = util.file_strip_ext(util.file_name(srcfile)) .. ".mkv"
local outfile = cachedir .. "/" .. filename
